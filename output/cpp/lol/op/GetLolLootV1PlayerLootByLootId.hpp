...
namespace lol {
  ...
  {
    try {
      return Result<LolLootPlayerLoot> {
        _client.https.request("get", "/lol-loot/v1/player-loot/"+to_string(lootId)+"?" +
      ...
      return Result<LolLootPlayerLoot> { Error { to_string(e.code().value()), -1, e.code().message() } };
    }
  }
    ...
    _client.httpsa.request("get", "/lol-loot/v1/player-loot/"+to_string(lootId)+"?" +
      SimpleWeb::QueryString::create(Args2Headers({  })), 
        ...
        {"Authorization", _client.auth},  }),[cb,&_client](std::shared_ptr<HttpsClient::Response> response, const SimpleWeb::error_code &e) {
          if(!e)
            cb(_client, Result<LolLootPlayerLoot> { response });
          else
            cb(_client,Result<LolLootPlayerLoot> { Error { to_string(e.value()), -1, e.message() } });
        });
  }
}