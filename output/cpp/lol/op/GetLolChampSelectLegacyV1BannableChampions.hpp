  ...
  inline Result<LolChampSelectLegacyChampSelectBannableChampions> GetLolChampSelectLegacyV1BannableChampions(LeagueClient& _client)
  {
    try {
      return Result<LolChampSelectLegacyChampSelectBannableChampions> {
        _client.https.request("get", "/lol-champ-select-legacy/v1/bannable-champions?" +
      ...
      return Result<LolChampSelectLegacyChampSelectBannableChampions> { Error { to_string(e.code().value()), -1, e.code().message() } };
    }
  }
    ...
    _client.httpsa.request("get", "/lol-champ-select-legacy/v1/bannable-champions?" +
      SimpleWeb::QueryString::create(Args2Headers({  })), 
        ...
        {"Authorization", _client.auth},  }),[cb,&_client](std::shared_ptr<HttpsClient::Response> response, const SimpleWeb::error_code &e) {
          if(!e)
            cb(_client, Result<LolChampSelectLegacyChampSelectBannableChampions> { response });
          else
            cb(_client,Result<LolChampSelectLegacyChampSelectBannableChampions> { Error { to_string(e.value()), -1, e.message() } });
        });
  }
}