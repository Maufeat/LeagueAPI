  ...
  inline Result<PluginManagerResource> GetPluginManagerV1Status(LeagueClient& _client)
  {
    try {
      return Result<PluginManagerResource> {
        _client.https.request("get", "/plugin-manager/v1/status?" +
      ...
      return Result<PluginManagerResource> { Error { to_string(e.code().value()), -1, e.code().message() } };
    }
  }
    ...
    _client.httpsa.request("get", "/plugin-manager/v1/status?" +
      SimpleWeb::QueryString::create(Args2Headers({  })), 
        ...
        {"Authorization", _client.auth},  }),[cb,&_client](std::shared_ptr<HttpsClient::Response> response, const SimpleWeb::error_code &e) {
          if(!e)
            cb(_client, Result<PluginManagerResource> { response });
          else
            cb(_client,Result<PluginManagerResource> { Error { to_string(e.value()), -1, e.message() } });
        });
  }
}