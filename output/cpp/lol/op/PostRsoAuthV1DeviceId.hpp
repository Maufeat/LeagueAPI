  ...
  inline Result<RsoAuthDeviceId> PostRsoAuthV1DeviceId(LeagueClient& _client)
  {
    try {
      return Result<RsoAuthDeviceId> {
        _client.https.request("post", "/rso-auth/v1/device-id?" +
      ...
      return Result<RsoAuthDeviceId> { Error { to_string(e.code().value()), -1, e.code().message() } };
    }
  }
    ...
    _client.httpsa.request("post", "/rso-auth/v1/device-id?" +
      SimpleWeb::QueryString::create(Args2Headers({  })), 
        ...
        {"Authorization", _client.auth},  }),[cb,&_client](std::shared_ptr<HttpsClient::Response> response, const SimpleWeb::error_code &e) {
          if(!e)
            cb(_client, Result<RsoAuthDeviceId> { response });
          else
            cb(_client,Result<RsoAuthDeviceId> { Error { to_string(e.value()), -1, e.message() } });
        });
  }
}