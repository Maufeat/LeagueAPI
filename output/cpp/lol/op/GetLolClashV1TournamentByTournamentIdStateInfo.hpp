  ...
  Result<LolClashTournamentStateInfo> GetLolClashV1TournamentByTournamentIdStateInfo(const LeagueClient& _client, const int64_t& tournamentId)
  {
    ...
    try {
      return Result<LolClashTournamentStateInfo> {
        _client_.request("get", "/lol-clash/v1/tournament/"+to_string(tournamentId)+"/stateInfo?" +
      ...
      return Result<LolClashTournamentStateInfo> { Error { to_string(e.code().value()), -1, e.what() } };
    }
  }
}