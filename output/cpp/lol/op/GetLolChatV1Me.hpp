  ...
  inline Result<LolChatUserResource> GetLolChatV1Me(LeagueClient& _client)
  {
    try {
      return Result<LolChatUserResource> {
        _client.https.request("get", "/lol-chat/v1/me?" +
      ...
      return Result<LolChatUserResource> { Error { to_string(e.code().value()), -1, e.code().message() } };
    }
  }
    ...
    _client.httpsa.request("get", "/lol-chat/v1/me?" +
      SimpleWeb::QueryString::create(Args2Headers({  })), 
        ...
        {"Authorization", _client.auth},  }),[cb,&_client](std::shared_ptr<HttpsClient::Response> response, const SimpleWeb::error_code &e) {
          if(!e)
            cb(_client, Result<LolChatUserResource> { response });
          else
            cb(_client,Result<LolChatUserResource> { Error { to_string(e.value()), -1, e.message() } });
        });
  }
}