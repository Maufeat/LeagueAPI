  ...
  inline Result<std::vector<LolChatBlockedPlayerResource>> GetLolChatV1BlockedPlayers(LeagueClient& _client)
  {
    try {
      return Result<std::vector<LolChatBlockedPlayerResource>> {
        _client.https.request("get", "/lol-chat/v1/blocked-players?" +
      ...
      return Result<std::vector<LolChatBlockedPlayerResource>> { Error { to_string(e.code().value()), -1, e.code().message() } };
    }
  }
    ...
    _client.httpsa.request("get", "/lol-chat/v1/blocked-players?" +
      SimpleWeb::QueryString::create(Args2Headers({  })), 
        ...
        {"Authorization", _client.auth},  }),[cb,&_client](std::shared_ptr<HttpsClient::Response> response, const SimpleWeb::error_code &e) {
          if(!e)
            cb(_client, Result<std::vector<LolChatBlockedPlayerResource>> { response });
          else
            cb(_client,Result<std::vector<LolChatBlockedPlayerResource>> { Error { to_string(e.value()), -1, e.message() } });
        });
  }
}