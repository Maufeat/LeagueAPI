  ...
  inline Result<ClubPreferences> PatchLolClubsV1ClubsMembershipPreferences(LeagueClient& _client, const ClubPreferences& preferences)
  {
    try {
      return Result<ClubPreferences> {
        _client.https.request("patch", "/lol-clubs/v1/clubs/membership/preferences?" +
      ...
      return Result<ClubPreferences> { Error { to_string(e.code().value()), -1, e.code().message() } };
    }
  }
    ...
    _client.httpsa.request("patch", "/lol-clubs/v1/clubs/membership/preferences?" +
      SimpleWeb::QueryString::create(Args2Headers({  })), 
        ...
        {"Authorization", _client.auth},  }),[cb,&_client](std::shared_ptr<HttpsClient::Response> response, const SimpleWeb::error_code &e) {
          if(!e)
            cb(_client, Result<ClubPreferences> { response });
          else
            cb(_client,Result<ClubPreferences> { Error { to_string(e.value()), -1, e.message() } });
        });
  }
}