  ...
  inline Result<LolPftPFTSurvey> GetLolPftV2Survey(LeagueClient& _client)
  {
    try {
      return Result<LolPftPFTSurvey> {
        _client.https.request("get", "/lol-pft/v2/survey?" +
      ...
      return Result<LolPftPFTSurvey> { Error { to_string(e.code().value()), -1, e.code().message() } };
    }
  }
    ...
    _client.httpsa.request("get", "/lol-pft/v2/survey?" +
      SimpleWeb::QueryString::create(Args2Headers({  })), 
        ...
        {"Authorization", _client.auth},  }),[cb,&_client](std::shared_ptr<HttpsClient::Response> response, const SimpleWeb::error_code &e) {
          if(!e)
            cb(_client, Result<LolPftPFTSurvey> { response });
          else
            cb(_client,Result<LolPftPFTSurvey> { Error { to_string(e.value()), -1, e.message() } });
        });
  }
}