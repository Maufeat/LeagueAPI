  ...
  inline Result<std::vector<LolChatConversationResource>> GetLolChatV1Conversations(LeagueClient& _client)
  {
    try {
      return Result<std::vector<LolChatConversationResource>> {
        _client.https.request("get", "/lol-chat/v1/conversations?" +
      ...
      return Result<std::vector<LolChatConversationResource>> { Error { to_string(e.code().value()), -1, e.code().message() } };
    }
  }
    ...
    _client.httpsa.request("get", "/lol-chat/v1/conversations?" +
      SimpleWeb::QueryString::create(Args2Headers({  })), 
        ...
        {"Authorization", _client.auth},  }),[cb,&_client](std::shared_ptr<HttpsClient::Response> response, const SimpleWeb::error_code &e) {
          if(!e)
            cb(_client, Result<std::vector<LolChatConversationResource>> { response });
          else
            cb(_client,Result<std::vector<LolChatConversationResource>> { Error { to_string(e.value()), -1, e.message() } });
        });
  }
}