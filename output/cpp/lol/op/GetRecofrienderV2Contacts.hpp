...
namespace lol {
  inline Result<std::vector<RecofrienderContactResource>> GetRecofrienderV2Contacts(const LeagueClient& _client)
  {
    HttpsClient _client_(_client.host, false);
    try {
      return Result<std::vector<RecofrienderContactResource>> {
        _client_.request("get", "/recofriender/v2/contacts?" +
