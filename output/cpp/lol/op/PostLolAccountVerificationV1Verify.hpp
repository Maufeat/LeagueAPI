  ...
  inline Result<LolAccountVerificationVerifyResponse> PostLolAccountVerificationV1Verify(LeagueClient& _client, const LolAccountVerificationVerifyRequest& VerifyRequest)
  {
    try {
      return Result<LolAccountVerificationVerifyResponse> {
        _client.https.request("post", "/lol-account-verification/v1/verify?" +
      ...
      return Result<LolAccountVerificationVerifyResponse> { Error { to_string(e.code().value()), -1, e.code().message() } };
    }
  }
    ...
    _client.httpsa.request("post", "/lol-account-verification/v1/verify?" +
      SimpleWeb::QueryString::create(Args2Headers({  })), 
        ...
        {"Authorization", _client.auth},  }),[cb,&_client](std::shared_ptr<HttpsClient::Response> response, const SimpleWeb::error_code &e) {
          if(!e)
            cb(_client, Result<LolAccountVerificationVerifyResponse> { response });
          else
            cb(_client,Result<LolAccountVerificationVerifyResponse> { Error { to_string(e.value()), -1, e.message() } });
        });
  }
}