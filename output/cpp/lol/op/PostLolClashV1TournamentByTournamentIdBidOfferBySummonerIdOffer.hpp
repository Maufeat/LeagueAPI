...
namespace lol {
  Result<json> PostLolClashV1TournamentByTournamentIdBidOfferBySummonerIdOffer(const LeagueClient& _client, const int64_t& tournamentId, const uint64_t& summonerId)
  {
    ...
    return _client_.request("post", "/lol-clash/v1/tournament/"+to_string(tournamentId)+"/bid-offer/"+to_string(summonerId)+"/offer?" + SimpleWeb::QueryString::create(Args2Headers({  })), "",
