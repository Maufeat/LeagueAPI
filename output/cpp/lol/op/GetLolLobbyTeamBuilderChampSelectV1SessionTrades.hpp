  ...
  inline Result<std::vector<LolLobbyTeamBuilderChampSelectTradeContract>> GetLolLobbyTeamBuilderChampSelectV1SessionTrades(LeagueClient& _client)
  {
    try {
      return Result<std::vector<LolLobbyTeamBuilderChampSelectTradeContract>> {
        _client.https.request("get", "/lol-lobby-team-builder/champ-select/v1/session/trades?" +
      ...
      return Result<std::vector<LolLobbyTeamBuilderChampSelectTradeContract>> { Error { to_string(e.code().value()), -1, e.code().message() } };
    }
  }
    ...
    _client.httpsa.request("get", "/lol-lobby-team-builder/champ-select/v1/session/trades?" +
      SimpleWeb::QueryString::create(Args2Headers({  })), 
        ...
        {"Authorization", _client.auth},  }),[cb,&_client](std::shared_ptr<HttpsClient::Response> response, const SimpleWeb::error_code &e) {
          if(!e)
            cb(_client, Result<std::vector<LolLobbyTeamBuilderChampSelectTradeContract>> { response });
          else
            cb(_client,Result<std::vector<LolLobbyTeamBuilderChampSelectTradeContract>> { Error { to_string(e.value()), -1, e.message() } });
        });
  }
}