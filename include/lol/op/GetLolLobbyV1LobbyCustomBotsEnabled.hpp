...
namespace lol {
  template<typename T>
  inline Result<bool> GetLolLobbyV1LobbyCustomBotsEnabled(T& _client)
  {
    try {
  ...
  template<typename T>
  inline void GetLolLobbyV1LobbyCustomBotsEnabled(T& _client, std::function<void(T&, const Result<bool>&)> cb)
  {
