...
namespace lol {
  template<typename T>
  inline Result<std::string> GetLolChatV1ConversationsNotify(T& _client)
  {
    try {
  ...
  template<typename T>
  inline void GetLolChatV1ConversationsNotify(T& _client, std::function<void(T&, const Result<std::string>&)> cb)
  {
