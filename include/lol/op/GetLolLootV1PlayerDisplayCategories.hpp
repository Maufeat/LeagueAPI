...
namespace lol {
  template<typename T>
  inline Result<std::vector<std::string>> GetLolLootV1PlayerDisplayCategories(T& _client)
  {
    try {
  ...
  template<typename T>
  inline void GetLolLootV1PlayerDisplayCategories(T& _client, std::function<void(T&, const Result<std::vector<std::string>>&)> cb)
  {
