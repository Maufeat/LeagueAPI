...
namespace lol {
  template<typename T>
  inline Result<std::vector<LolLobbyLobbyInvitation>> GetLolLobbyV1LobbyInvitations(T& _client)
  {
    try {
  ...
  template<typename T>
  inline void GetLolLobbyV1LobbyInvitations(T& _client, std::function<void(T&, const Result<std::vector<LolLobbyLobbyInvitation>>&)> cb)
  {
