...
namespace lol {
  template<typename T>
  inline Result<std::string> GetLolCollectionsV1InventoriesBySummonerIdVerification(T& _client, const uint64_t& summonerId)
  {
    try {
  ...
  template<typename T>
  inline void GetLolCollectionsV1InventoriesBySummonerIdVerification(T& _client, const uint64_t& summonerId, std::function<void(T&, const Result<std::string>&)> cb)
  {
