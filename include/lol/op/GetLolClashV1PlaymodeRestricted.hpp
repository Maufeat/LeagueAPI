...
namespace lol {
  template<typename T>
  inline Result<bool> GetLolClashV1PlaymodeRestricted(T& _client)
  {
    try {
  ...
  template<typename T>
  inline void GetLolClashV1PlaymodeRestricted(T& _client, std::function<void(T&, const Result<bool>&)> cb)
  {
