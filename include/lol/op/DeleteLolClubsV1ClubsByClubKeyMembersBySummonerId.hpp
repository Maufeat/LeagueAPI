...
namespace lol {
  template<typename T>
  inline Result<ClubMemberLists> DeleteLolClubsV1ClubsByClubKeyMembersBySummonerId(T& _client, const std::string& clubKey, const uint64_t& summonerId)
  {
    try {
  ...
  template<typename T>
  inline void DeleteLolClubsV1ClubsByClubKeyMembersBySummonerId(T& _client, const std::string& clubKey, const uint64_t& summonerId, std::function<void(T&, const Result<ClubMemberLists>&)> cb)
  {
