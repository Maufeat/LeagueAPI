...
namespace lol {
  template<typename T>
  inline Result<LolChampSelectChampSelectTradeContract> GetLolChampSelectV1SessionTradesById(T& _client, const int64_t& id)
  {
    try {
  ...
  template<typename T>
  inline void GetLolChampSelectV1SessionTradesById(T& _client, const int64_t& id, std::function<void(T&, const Result<LolChampSelectChampSelectTradeContract>&)> cb)
  {
