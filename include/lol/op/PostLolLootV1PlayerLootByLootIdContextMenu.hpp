...
namespace lol {
  template<typename T>
  inline Result<std::vector<LolLootContextMenu>> PostLolLootV1PlayerLootByLootIdContextMenu(T& _client, const std::string& lootId)
  {
    try {
  ...
  template<typename T>
  inline void PostLolLootV1PlayerLootByLootIdContextMenu(T& _client, const std::string& lootId, std::function<void(T&, const Result<std::vector<LolLootContextMenu>>&)> cb)
  {
